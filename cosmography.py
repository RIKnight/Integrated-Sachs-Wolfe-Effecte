...
def DAngular(zMax,omegaM,omegaL,nSteps,DH):
  """ 
...
def DLuminosity(zMax,omegaM,omegaL,nSteps,DH):
  ...
  """
  zVals, distances = DAngular(zMax,omegaM,omegaL,nSteps,DH)
  for i in range(1,nSteps):
    distances[i] = distances[i]*(1+zVals[i])**2
  return zVals, distances

def ApMagnitude(zMax,omegaM,omegaL,nSteps,DH):
  """ 
  ...
  """
  MStar = -20.6
  zVals, distances = DLuminosity(zMax,omegaM,omegaL,nSteps,DH)
  magnitudes = np.zeros(nSteps)
  for i in range(1,nSteps):
...
def AngSize(zMax,omegaM,omegaL,nSteps,DH):
  """ 
    angsize = size/D_A 
    DH is the Hubble distance c/H_0 in Mpc
    uses DAngular
  ...
  zVals, distances = DAngular(zMax,omegaM,omegaL,nSteps,DH)
  angSize = np.zeros(nSteps)
  for i in range(1,nSteps):
    angSize[i] = linearSize/distances[i]*206265
  return zVals, angSize

def LookBackTime(zMax,omegaM,omegaL,nSteps,H_0):
  """
    equation 30, Hogg
  ...
  for i in range(1,nSteps):
    zVals[i] = i*stepSize
    times[i] = times[i-1] + stepSize/E(zVals[i],omegaM,0,omegaL)/(1+zVals[i])/H_0
  # convert from seconds*Mpc/km to Gyrs
  return zVals, times*conv

def ComovingDistance(zMax,omegaM,omegaL,nSteps,H_0):
  """
    H_0 is the Hubble constant in km/s/Mpc
    implements numeric integration by adding up small increments
  ...
  for i in range(1,nSteps):
    zVals[i] = i*stepSize
    distances[i] = distances[i-1] + stepSize/E(zVals[i],omegaM,0,omegaL)/H_0
